...
/**
 * This interface is used to represent an HTTP message that has been intercepted by Burp Proxy. Extensions can register an <code>IProxyListener</code> to receive details of proxy messages using this
 * interface. *
 */
public interface IInterceptedProxyMessage {
   ...
   *
   * @return An identifier that is unique to a single request/response pair. Extensions can use this to correlate details of requests and responses and perform processing on the response message
   *         accordingly.
   */
   ...
   *
   * @return An <code>IHttpRequestResponse</code> object containing details of the intercepted message.
   */
  IHttpRequestResponse getMessageInfo();

  /**
   * This method retrieves the currently defined interception action. The default action is <code>ACTION_FOLLOW_RULES</code>. If multiple proxy listeners are registered, then other listeners may
   * already have modified the interception action before it reaches the current listener. This method can be used to determine whether this has occurred.
   *
