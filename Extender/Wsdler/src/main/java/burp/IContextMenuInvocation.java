...
/**
 * This interface is used when Burp calls into an extension-provided <code>IContextMenuFactory</code> with details of a context menu invocation. The custom context menu factory can query this
 * interface to obtain details of the invocation event, in order to determine what menu items should be displayed.
 */
public interface IContextMenuInvocation {
   ...
   *
   * @return A flag indicating the Burp tool within which the context menu was invoked. Burp tool flags are defined in the <code>IBurpExtenderCallbacks</code> interface.
   */
  int getToolFlag();
   ...
   *
   * @return An index indicating the context within which the menu was invoked. The indices used are defined within this interface.
   */
   ...
   *
   * @return An int[2] array containing the start and end offsets of the user's selection in the current message. If the user has not made any selection in the current message, both offsets indicate
   *         the position of the caret within the editor. If the menu is not being invoked from a message editor, the method returns <code>null</code>.
   */
   ...
   *
   * @return An array of <code>IHttpRequestResponse</code> objects representing the items that were shown or selected by the user when the context menu was invoked. This method returns
   ...
   *
   * @return An array of <code>IScanIssue</code> objects representing the issues that were selected by the user when the context menu was invoked. This method returns <code>null</code> if no Scanner
