...
/**
 * This interface is used to retrieve details of items in the Burp Scanner active scan queue. Extensions can obtain references to scan queue items by calling
...
public interface IScanQueueItem {
  ...
  /**
   * This method returns details of the issues generated for the scan queue item. <b>Note:</b> different items within the scan queue may contain duplicated versions of the same issues - for example,
   * if the same request has been scanned multiple times. Duplicated issues are consolidated in the main view of scan results. Extensions can register an <code>IScannerListener</code> to get details
   * only of unique, newly discovered Scanner issues post-consolidation.
   *
