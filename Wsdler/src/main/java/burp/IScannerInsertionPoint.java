...
/**
 * This interface is used to define an insertion point for use by active Scanner checks. Extensions can obtain instances of this interface by registering an <code>IScannerCheck</code>, or can create
 * instances for use by Burp's own scan checks by registering an <code>IScannerInsertionPointProvider</code>.
 */
public interface IScannerInsertionPoint {
  ...
  /**
   * Used to indicate where the insertion point is provided by an extension-registered <code>IScannerInsertionPointProvider</code>.
   */
   ...
   *
   * @return the base value that appears in this insertion point in the base request being scanned, or <code>null</code> if there is no value in the base request that corresponds to this insertion
   *         point.
   */
  String getBaseValue();

  /**
   * This method is used to build a request with the specified payload placed into the insertion point. Any necessary adjustments to the Content-Length header will be made by the Scanner itself when
   * the request is issued, and there is no requirement for the insertion point to do this. <b>Note:</b> Burp's built-in scan checks do not apply any payload encoding (such as URL-encoding) when
   * dealing with an extension-provided insertion point. Custom insertion points are responsible for performing any data encoding that is necessary given the nature and location of the insertion
   * point.
   *
  ...
  /**
   * This method is used to determine the offsets of the payload value within the request, when it is placed into the insertion point. Scan checks may invoke this method when reporting issues, so as
   * to highlight the relevant part of the request within the UI.
   *
   * @param payload The payload that should be placed into the insertion point.
   * @return An int[2] array containing the start and end offsets of the payload within the request, or null if this is not applicable (for example, where the insertion point places a payload into a
   *         serialized data structure, the raw payload may not literally appear anywhere within the resulting request).
   */
   ...
   *
   * @return The type of the insertion point. Available types are defined in this interface.
   */
