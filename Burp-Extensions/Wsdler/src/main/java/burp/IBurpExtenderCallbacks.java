 ...
 *
 * When an extension is loaded, Burp invokes its <code>registerExtenderCallbacks()</code> method and passes an instance of the <code>IBurpExtenderCallbacks</code> interface. The extension may then
 * invoke the methods of this interface as required in order to extend Burp's functionality.
 */
public interface IBurpExtenderCallbacks {
  ...
  /**
   * This method is used to obtain an <code>IExtensionHelpers</code> object, which can be used by the extension to perform numerous useful tasks.
   *
  ...
  /**
   * This method is used to obtain the current extension's standard output stream. Extensions should write all output to this stream, allowing the Burp user to configure how that output is handled
   * from within the UI.
   *
  ...
  /**
   * This method is used to obtain the current extension's standard error stream. Extensions should write all error messages to this stream, allowing the Burp user to configure how that output is
   * handled from within the UI.
   *
  ...
  /**
   * This method is used to register a listener which will be notified of requests and responses made by any Burp tool. Extensions can perform custom analysis or modification of these messages by
   * registering an HTTP listener.
   *
   * @param listener An object created by the extension that implements the <code>IHttpListener</code> interface.
   */
  void registerHttpListener(IHttpListener listener);

  /**
   * This method is used to register a listener which will be notified of requests and responses being processed by the Proxy tool. Extensions can perform custom analysis or modification of these
   * messages, and control in-UI message interception, by registering a proxy listener.
   *
   * @param listener An object created by the extension that implements the <code>IProxyListener</code> interface.
   */
  void registerProxyListener(IProxyListener listener);

  /**
   * This method is used to register a listener which will be notified of new issues that are reported by the Scanner tool. Extensions can perform custom analysis or logging of Scanner issues by
   * registering a Scanner listener.
   *
   * @param listener An object created by the extension that implements the <code>IScannerListener</code> interface.
   */
   ...
   *
   * @param listener An object created by the extension that implements the <code>IScopeChangeListener</code> interface.
   */
  ...
  /**
   * This method is used to register a factory for custom message editor tabs. For each message editor that already exists, or is subsequently created, within Burp, the factory will be asked to
   * provide a new instance of an <code>IMessageEditorTab</code> object, which can provide custom rendering or editing of HTTP messages.
   *
   * @param factory An object created by the extension that implements the <code>IMessageEditorTabFactory</code> interface.
   */
  void registerMessageEditorTabFactory(IMessageEditorTabFactory factory);

  /**
   * This method is used to register a provider of Scanner insertion points. For each base request that is actively scanned, Burp will ask the provider to provide any custom scanner insertion points
   * that are appropriate for the request.
   *
   * @param provider An object created by the extension that implements the <code>IScannerInsertionPointProvider</code> interface.
   */
  ...
  /**
   * This method is used to register a custom Intruder payload processor. Each registered processor will be available within the Intruder UI for the user to select as the action for a payload
   * processing rule.
   *
   * @param processor An object created by the extension that implements the <code>IIntruderPayloadProcessor</code> interface.
   */
  void registerIntruderPayloadProcessor(IIntruderPayloadProcessor processor);

  /**
   * This method is used to register a custom session handling action. Each registered action will be available within the session handling rule UI for the user to select as a rule action. Users can
   * choose to invoke an action directly in its own right, or following execution of a macro.
   *
   * @param action An object created by the extension that implements the <code>ISessionHandlingAction</code> interface.
   */
   ...
   *
   * @param tab An object created by the extension that implements the <code>ITab</code> interface.
   */
   ...
   *
   * @param controller An object created by the extension that implements the <code>IMessageEditorController</code> interface. This parameter is optional and may be <code>null</code>. If it is
   *                   provided, then the message editor will query the controller when required to obtain details about the currently displayed message, including the <code>IHttpService</code> for
   *                   the message, and the associated request or response message. If a controller is not provided, then the message editor will not support context menu actions, such as sending
   ...
   * @return An object that implements the <code>IMessageEditor</code> interface, and which the extension can use in its own UI.
   */
  ...
  /**
   * This method is used to load configuration settings for the extension that were saved using the method <code>saveExtensionSetting()</code>.
   *
   * @param name The name of the setting.
   * @return The value of the setting, or <code>null</code> if no value is set.
   */
   ...
   *
   * @return An object that implements the <code>ITextEditor</code> interface, and which the extension can use in its own UI.
   */
   ...
   * @param tabCaption An optional caption which will appear on the Repeater tab containing the request. If this value is <code>null</code> then a default tab index will be displayed.
   */
  void sendToRepeater(
      String host,
      int port,
  ...
  /**
   * This method can be used to send an HTTP request to the Burp Intruder tool. The request will be displayed in the user interface, and markers for attack payloads will be placed into default
  ...
  /**
   * This method can be used to send an HTTP request to the Burp Intruder tool. The request will be displayed in the user interface, and markers for attack payloads will be placed into the specified
   * locations within the request.
   *
   * @param host                   The hostname of the remote HTTP server.
   * @param port                   The port of the remote HTTP server.
   * @param useHttps               Flags whether the protocol is HTTPS or HTTP.
   * @param request                The full HTTP request.
   * @param payloadPositionOffsets A list of index pairs representing the payload positions to be used. Each item in the list must be an int[2] array containing the start and end offsets for the
   *                               payload position.
   */
  void sendToIntruder(
      String host,
  ...
  /**
   * This method can be used to send a seed URL to the Burp Spider tool. If the URL is not within the current Spider scope, the user will be asked if they wish to add the URL to the scope. If the
   * Spider is not currently running, it will be started. The seed URL will be requested, and the Spider will process the application's response in the normal way.
   *
   ...
   * @param useHttps              Flags whether the protocol is HTTPS or HTTP.
   * @param request               The full HTTP request.
   * @param insertionPointOffsets A list of index pairs representing the positions of the insertion points that should be scanned. Each item in the list must be an int[2] array containing the start
   *                              and end offsets for the insertion point.
   * @return The resulting scan queue item.
   */
  IScanQueueItem doActiveScan(
      String host,
   ...
   * @param request     The full HTTP request.
   * @return An object that implements the <code>IHttpRequestResponse</code> interface, and which the extension can query to obtain the details of the response.
   */
   ...
   * @return Returns <code>true</code> if the URL is within the current Suite-wide scope.
   */
   ...
   *
   * @param urlPrefix This parameter can be used to specify a URL prefix, in order to extract a specific subset of the site map. The method performs a simple case-sensitive text match, returning all
   *                  site map items whose URL begins with the specified prefix. If this parameter is null, the entire site map is returned.
   ...
   *
   * @param urlPrefix This parameter can be used to specify a URL prefix, in order to extract a specific subset of scan issues. The method performs a simple case-sensitive text match, returning all
   *                  scan issues whose URL begins with the specified prefix. If this parameter is null, all issues are returned.
  ...
  /**
   * This method is used to update the contents of Burp's session handling cookie jar. Extensions that provide an <code>ISessionHandlingAction</code> can query and update the cookie jar in order to
   * handle unusual session handling mechanisms.
   *
   * @param cookie An <code>ICookie</code> object containing details of the cookie to be updated. If the cookie jar already contains a cookie that matches the specified domain and name, then that
   *               cookie will be updated with the new value and expiration, unless the new value is <code>null</code>, in which case the cookie will be removed. If the cookie jar does not already
   *               contain a cookie that matches the specified domain and name, then the cookie will be added.
   */
  ...
  /**
   * This method can be used to restore Burp's state from a specified saved state file. This method blocks until the restore operation is completed, and must not be called from the event dispatch
   * thread.
   *
  ...
  /**
   * This method can be used to save Burp's state to a specified file. This method blocks until the save operationName is completed, and must not be called from the event dispatch thread.
   *
  ...
  /**
   * This method is used to create a temporary file on disk containing the provided data. Extensions can use temporary files for long-term storage of runtime data, avoiding the need to retain that
   * data in memory.
   *
  ...
  /**
   * This method is used to apply markers to an HTTP request or response, at offsets into the message that are relevant for some particular purpose. Markers are used in various situations, such as
   * specifying Intruder payload positions, Scanner insertion points, and highlights in Scanner issues.
   *
   ...
   *
   * @param toolFlag A flag identifying a Burp tool ( <code>TOOL_PROXY</code>, <code>TOOL_SCANNER</code>, etc.). Tool flags are defined within this interface.
   ...
   * @return An array of: <code>String[] { name, value, type }</code> containing details of the parameters contained within the request.
   ...
   * @deprecated Use <code>IExtensionHelpers.analyzeRequest()</code> or <code>IExtensionHelpers.analyzeResponse()</code> instead.
   */
  @Deprecated
